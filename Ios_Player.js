// ==UserScript==
// @name         iOS 玻璃质感视频播放器
// @namespace    https://local.script/ios-glass-player
// @version      1.5.0
// @description  iOS 风格高斯模糊玻璃视频控件：后退/快进、加粗胶囊进度条、静音、画中画/全屏（自动锁横屏+返回按钮）、触摸或靠近按钮立即泛光放大、点空白处即时切换控件显示/隐藏、右上角更多菜单（下载视频/复制视频链接，按格式自动判断是否可点）
// @author       @520luo
// @match        *://*/*
...
`;
  ...
  function buildControls() {
    ...
    return {
      ...
      pipBtn, fullscreenBtn, muteBtn, returnBtn, moreBtn, menu, downloadItem, copyItem
    };
  }
  ...
  function wireEvents(video, wrapper, refs) {
    ...
    const STREAMING_EXT = ['m3u8', 'mpd']; // HLS / DASH 清单，不是单个可下载的视频文件
    ...
    function triggerDownload(url, filename) {
      try {
        ...
        a.click();
        a.remove();
      } catch (_) { /* 部分跨域直链浏览器会改成直接打开，而不是下载，属于浏览器安全限制 */ }
    }
    ...
    let blobSniffToken = 0; // 避免菜单多次打开时，旧的异步检测结果覆盖新的
    function updateMenuState() {
      const url = getVideoSourceUrl();
      const myToken = ++blobSniffToken;

      copyItem.disabled = !url; // 复制链接：只要提取到地址就能点，跟格式无关

      if (!url) {
        downloadItem.disabled = true;
        return;
      }
      downloadItem.dataset.url = url;

      if (url.startsWith('blob:')) {
        // blob: 地址（多见于用 MSE 喂数据的播放器）拿不到扩展名，
        // 异步嗅探一下真实的媒体类型，确认是 mp4/webm 这类才允许下载
        downloadItem.disabled = true;
        fetch(url)
          .then((r) => r.blob())
          .then((blob) => {
            if (myToken !== blobSniffToken) return; // 菜单已经重新打开过，结果过期了
            const t = (blob.type || '').toLowerCase();
            const ok = ['mp4', 'webm', 'matroska', 'quicktime', 'mpeg'].some((k) => t.includes(k));
            downloadItem.disabled = !ok;
          })
      ...
      if (STREAMING_EXT.includes(ext)) downloadItem.disabled = true;
      else if (DOWNLOADABLE_EXT.includes(ext)) downloadItem.disabled = false;
      else downloadItem.disabled = true; // 无法识别的格式，保守起见不开放下载
    }
    ...
    downloadItem.addEventListener('click', (e) => {
      ...
      const url = downloadItem.dataset.url;
      if (!url) return;
      let ext = '';
